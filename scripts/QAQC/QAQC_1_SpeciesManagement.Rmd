...
```{r}
# Blank data frame
errors_blank_species <- data.frame("SavedQuery" = "", "Code" = "", "Error" = "", "Fixed" = "", "Explanation" = "", "Queryers" = "")
...
qaqc_species <- function(data, query, query_message, values_check) {
  ...
  errors <- data %>%  
    ...
    select("SavedQuery", "Code", "Error", "Fixed", "Explanation", "Queryers")
  
  # Next, check if there are duplicate errors
  errors_temp <- errors %>% 
    get_dupes(SavedQuery, Code, Error)
...
for (i in 2:nrow(errors_SpeciesReport)) {
  if (errors_SpeciesReport[i, "Code"] != errors_SpeciesReport[i - 1, "Code"]) {
    errors_SpeciesReport_blanks <- rbind(errors_SpeciesReport_blanks, errors_blank_species, errors_SpeciesReport[i, ])
  } else {
