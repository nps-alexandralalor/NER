...
```{r}
# Blank data frame
errors_blank <- data.frame("SavedQuery" = "", "SampleEvent_TreatmentUnit" = "", "MacroPlot.Name" = "", "Date" = "", "Error" = "", "Fixed" = "", "Explanation" = "", "Queryers" = "") %>%
...
### Selected year(s): [`r years`]{style="color:red"}
...
```{r}
# Combine
errors_Fuels1000_Complete <- unique(rbind(errors_Fuels1000_NoData, errors_Fuels1000_Quotes))

remove(errors_Fuels1000_NoData, errors_Fuels1000_Quotes)

if(nrow(errors_Fuels1000_Complete) > 1) {
  errors_Fuels1000_Complete <- errors_Fuels1000_Complete %>%
...
```{r}
# Combine
errors_FuelsDuffLitt_Complete <- unique(rbind(errors_FuelsDuffLitt_NoData, errors_FuelsDuffLitt_Quotes))

remove(errors_FuelsDuffLitt_NoData, errors_FuelsDuffLitt_Quotes)

if(nrow(errors_FuelsDuffLitt_Complete) > 1) {
  errors_FuelsDuffLitt_Complete <- errors_FuelsDuffLitt_Complete %>%
    filter(Error != "No Error")
} else {
  errors_FuelsDuffLitt_Complete <- errors_FuelsDuffLitt_Complete}
```
...
```{r}
# Combine
errors_FuelsFine_Complete <- unique(rbind(errors_FuelsFine_NoData, errors_FuelsFine_Quotes))

remove(errors_FuelsFine_NoData, errors_FuelsFine_Quotes)

if(nrow(errors_FuelsFine_Complete) > 1) {
  errors_FuelsFine_Complete <- errors_FuelsFine_Complete %>%
    filter(Error != "No Error")
} else {
  errors_FuelsFine_Complete <- errors_FuelsFine_Complete}
```
...
```{r}
# Combine
errors_PostBurn_Complete <- unique(rbind(errors_PostBurn_NoData, errors_PostBurn_Quotes))

remove(errors_PostBurn_NoData, errors_PostBurn_Quotes)

if(nrow(errors_PostBurn_Complete) > 1) {
  errors_PostBurn_Complete <- errors_PostBurn_Complete %>%
    filter(Error != "No Error")
} else {
  errors_PostBurn_Complete <- errors_PostBurn_Complete}
```
...
```{r}
# Combine
errors_Trees_Complete <- unique(rbind(errors_Trees_NoData, errors_Trees_Quotes))

remove(errors_Trees_NoData, errors_Trees_Quotes)

if(nrow(errors_Trees_Complete) > 1) {
  errors_Trees_Complete <- errors_Trees_Complete %>%
    filter(Error != "No Error")
} else {
  errors_Trees_Complete <- errors_Trees_Complete}
```
...
```{r}
# Combine
errors_Seedlings_Complete <- unique(rbind(errors_Seedlings_NoData, errors_Seedlings_Quotes))

remove(errors_Seedlings_NoData, errors_Seedlings_Quotes)

if(nrow(errors_Seedlings_Complete) > 1) {
  errors_Seedlings_Complete <- errors_Seedlings_Complete %>%
    filter(Error != "No Error")
} else {
  errors_Seedlings_Complete <- errors_Seedlings_Complete}
```
...
```{r}
# Combine
errors_Shrubs_Complete <- unique(rbind(errors_Shrubs_NoData, errors_Shrubs_Quotes))

remove(errors_Shrubs_NoData, errors_Shrubs_Quotes)

if(nrow(errors_Shrubs_Complete) > 1) {
  errors_Shrubs_Complete <- errors_Shrubs_Complete %>%
    filter(Error != "No Error")
} else {
  errors_Shrubs_Complete <- errors_Shrubs_Complete}
```
...
```{r}
# Combine
errors_HerbsSpComp_Complete <- unique(rbind(errors_HerbsSpComp_NoData, errors_HerbsSpComp_Quotes))

remove(errors_HerbsSpComp_NoData, errors_HerbsSpComp_Quotes)

if(nrow(errors_HerbsSpComp_Complete) > 1) {
  errors_HerbsSpComp_Complete <- errors_HerbsSpComp_Complete %>%
    filter(Error != "No Error")
} else {
  errors_HerbsSpComp_Complete <- errors_HerbsSpComp_Complete}
```
...
```{r}
errors_HerbsSpComp <- rbind(errors_HerbsSpComp_Complete, errors_blank, errors_HerbsSpComp_Header, errors_blank, errors_HerbsSpComp_Cover, errors_blank,  errors_HerbsSpComp_Status, errors_blank, errors_HerbsSpComp_Zero)

remove(errors_HerbsSpComp_Complete, errors_HerbsSpComp_Header, errors_HerbsSpComp_Cover,  errors_HerbsSpComp_Status, errors_HerbsSpComp_Zero)
```
...
```{r}
# Combine
errors_HerbsPoints_Complete <- unique(rbind(errors_HerbsPoints_NoData, errors_HerbsPoints_Quotes))

remove(errors_HerbsPoints_NoData, errors_HerbsPoints_Quotes)
