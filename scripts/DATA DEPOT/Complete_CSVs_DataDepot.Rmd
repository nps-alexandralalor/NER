...
```{r}
diffdf(Fuels1000_DataDepot_SHEN, Fuels1000_DataFFI)
```
...
HerbsSpComp_DataDepot <- HerbsSpComp_M %>% 
  mutate(SampleEvent_Date = as.Date(SampleEvent_Date, "%m/%d/%Y")) %>% 
  select(!c(
    Organization_Name, AdminUnit_Comment, Macroplot_Lat_dd, Macroplot_Long_dd, Macroplot_UtmX_m, Macroplot_UtmY_m, Macroplot_UtmZone, Macroplot_Datum, Macroplot_Pdop, Macroplot_Precision, Macroplot_DateIn, Macroplot_DateOut, Macroplot_FutureVisit, Macroplot_Elevation, Macroplot_ElevationUnits, Macroplot_Azimuth, Macroplot_Aspect, Macroplot_SlopeHill_pct, Macroplot_SlopeTransect_pct, Macroplot_LocatedBy, Macroplot_StartPoint, Macroplot_Directions, Macroplot_Comment, Macroplot_Metadata, SampleEvent_Who, SampleEvent_Comment, SampleEvent_UserVariables, SampleEvent_LegacyMonitoringStatus, SampleData_Executed, MethodData_CreatedDate, MethodData_UpdatedDate, MethodData_DeletedDate, MethodData_SppGuid, 
    Species_SourceEntityId, Species_LocalSpeciesId, Species_MasterSpeciesId, Species_AuxSpeciesId, Species_ITISTaxonomicSerialNumber, Species_SymbolLocal, Species_SymbolMaster, Species_CommonNameLocal, Species_CommonName, Species_Nativity, Species_LifeFormName, Species_Description, Species_LifeCycle, Species_Cultural, Species_Concern, Species_Retired, Species_Comment, Species_UV1, Species_UV2, Species_UV3
    )) %>% 
  relocate(Macroplot_Name, .after = Macroplot_UV8) %>%
  relocate(SampleEvent_TreatmentUnit, .before = SampleEvent_Date) %>% 
...
```{r}
diffdf(HerbsSpComp_DataDepot_SHEN, HerbsSpComp_DataFFI_SHEN)
```
...
```{r}
diffdf(HerbsPoints_DataDepot_SHEN, HerbsPoints_DataFFI)
```
