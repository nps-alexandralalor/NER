...
# Formatted
condformat2excel(errors_Fuels_CrossYear_blanks, paste0(path_errors, "Errors_DataEntry_CrossYear_Fuels.xlsx"), sheet_name = path_errors_name, overwrite_wb = TRUE)
```
...
data <- merge(Trees_data_CrossYear, data_temp_merge_Status, all = T) %>% 
  filter(MethodData_Status == "L")
query <- "Trees Status" 
query_message <- paste0("Status = ", data$MethodData_Status, ". ", "Death date")
...
for (i in 2:nrow(errors_Trees_CrossYear)) {
  if (errors_Trees_CrossYear[i, "MethodData_TagNo"] != errors_Trees_CrossYear[i - 1, "MethodData_TagNo"]) {
    errors_Trees_CrossYear_blanks <- rbind(errors_Trees_CrossYear_blanks, errors_blank, errors_Trees_CrossYear[i, ])
  } else {
...
# # Add conditional formating to ouput
errors_Trees_CrossYear_blanks <- condformat(errors_Trees_CrossYear_blanks[1:(nrow(errors_Trees_CrossYear_blanks)), 1:ncol(errors_Trees_CrossYear)]) %>%
  rule_fill_discrete(columns = 1:ncol(errors_Trees_CrossYear),
                     expression = SavedQuery != "",
                     colours = c("TRUE" = "#C6E0B4"),
                     na.value = "#FFFFFF")
```
...
# Formatted
# condformat2excel(errors_Trees_CrossYear_blanks, paste0(path_errors, "Errors_DataEntry_CrossYear_Trees.xlsx"), sheet_name = path_errors_name, overwrite_wb = TRUE)
```

## COMBINE ERRORS

```{r}
errors_CrossYear_blanks <- rbind(errors_Fuels_CrossYear_blanks, errors_blank, errors_Trees_CrossYear_blanks)
