...
data <- Trees_data %>% 
  ...
  rowwise() %>% 
  dplyr::mutate(
    DamCd = paste(unique(str_trim(c_across(starts_with("MethodData_DamCd"))))[
  ...
  ungroup()
query <- "Trees Damage" 
query_message <- paste0("Tag ", data$MethodData_TagNo, ". ", "Status = L. Damage Codes = ", data$DamCd, ". Valid code")  
...
data <- Trees_data %>% 
  rowwise() %>% 
  dplyr::mutate(
    DamCd = paste(unique(str_trim(c_across(starts_with("MethodData_DamCd")))), collapse = "; "),
    DamCd_missing = grepl("^\\s*;", DamCd)
  ) %>%
  ungroup()

query <- "Trees Damage" 
query_message <- paste0("Tag ", data$MethodData_TagNo, ". ", "Damage Codes = ", data$DamCd, ". Missing damage code")
...
data_temp <- Trees_data %>% 
  get_dupes(SampleEvent_Date, Macroplot_Name, SubFrac, TagNo)
...
# Set parameters 
data <- Trees_data %>%
  mutate(Post = ifelse((grepl("Post|Severity", Trees_data$MonitoringStatus_Name) == TRUE), 
                       TRUE, FALSE)) %>%
  filter(Post == FALSE)
...
query <- "Cvr SpComp Species"
query_message <- paste0("Species = ", data$Species.Symbol, ". Comment")
values_data <- data$Comment
values_valid <- ""
values_check <- values_data == values_valid
...
# Check for tag duplication and store results in new column
data_temp <- HerbsPoints_data %>% 
  get_dupes(SampleEvent_Date, Macroplot_Name, Transect, Point, Order)
...
data_temp <- data_temp %>% 
  mutate(Missing = abs(expected_sum - Point)) %>% 
  select(!Point)
...
data$Comment[is.na(data$Comment)] <- ""
query <- "Cvr Pts Species"
query_message <- paste0("Transect = ", data$Transect, ". ", "Point = ", data$Point, ". ", "Species = ", data$Species.Symbol, ". Comment")
values_data <- data$Comment
values_valid <- ""
values_check <- values_data == values_valid
