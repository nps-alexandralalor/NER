...
errors_blank <- data.frame("SavedQuery" = "", "AdministrationUnit_Name" = "", "SampleEvent_TreatmentUnit" = "", "Macroplot" = "", "SampleEventDate" = "", "Error" = "", "Fixed" = "", "Explanation" = "", "Queryers" = "") %>%
  mutate(SampleEventDate = as.Date(SampleEventDate))

# Create error group. Duplicate errors will be grouped by these columns.
errors_group <- c("SavedQuery", "AdministrationUnit_Name", "Macroplot", "SampleEventDate", "Error")
...
Fuels1000_data <- Fuels1000_all %>%
  filter(SampleEventDate_Year %in% target_years)
...
# Set parameters 
data <- Fuels1000_data %>% 
  filter(Purpose == "Inventory & Monitoring")
query <- "Fuel 1000 Header"
query_message <- "Number of Transects" 
values_data <- data$NumTran
values_valid <- 1
...
data_temp <- Fuels1000_data %>%
  get_dupes(SampleEventDate, Macroplot, Transect, LogNum, Dia, DecayCl)
...
data_temp1 <- Fuels1000_data %>%
  group_by(Macroplot, SampleEventDate, Transect, Slope) %>%
  dplyr::summarise("Macroplot" = unique(Macroplot),
            "Transect" = unique(Transect)) %>%
  mutate(Slope_1000 = Slope) %>%
  arrange(Macroplot, Transect) %>%
  select(!Slope)
...
# Set parameters
data <- merge(FuelsFine_data, data_temp, by = c("Macroplot", "SampleEventDate", "Transect"), all = T)
query <- "Fuel Fine Slope"
...
data_temp <- Trees_data %>% 
  get_dupes(SampleEventDate, Macroplot, SubFrac, TagNo)
...
# Set parameters 
data <- Trees_data %>%
  mutate(Post = ifelse((grepl("Post|Severity", Trees_data$MonStatus) == TRUE), 
                       TRUE, FALSE)) %>%
  filter(Post == FALSE)
...
# # Perform Rosner's Test for outliers using all values
# data_temp <- Trees_all %>%
#   filter(MonStatus %in% c("01Post", "02Post", "03Post", "04Post"),
#          !is.na(CharHt))
...
data_temp <- HerbsPoints_data %>% 
  get_dupes(SampleEventDate, Macroplot, Transect, Point, Order)
...
data_temp <- HerbsPoints_data %>%
  filter(is.na(Order)|Order == 1,
         Type == "Forest") %>% 
  group_by(Macroplot, SampleEventDate, Transect) %>% 
...
# Set parameters 
data <- merge(HerbsPoints_data, data_temp, by = c("Macroplot", "SampleEventDate", "Transect"), all = T) %>% 
  filter(Type == "Forest")
query <- "Cvr Pts Point" 
...
errors_all <- errors_all %>% 
  mutate(SampleEventDate = as.character(SampleEventDate))
