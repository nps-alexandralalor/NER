...
errors_blank <- data.frame("SavedQuery" = "", "Macroplot" = "", "Error" = "", "Fixed" = "", "Explanation" = "", "Queryers" = "")

# Create error group. Duplicate errors will be grouped by these columns.
errors_group <- c("SavedQuery", "Macroplot", "Error")
...
# Identify errors
errors_MacroplotReport_MonStatusOrd <- qaqc(data, query, query_message, values_check)
```
...
[Problem:]{.underline} Macroplot Report is missing information related to plot comments, or comments include symbols which will cause issues later on.
...
query <- "Macroplot Report - Comments"
...
-   Check that UV1 = Project Unit / Burn Unit
-   Filter for Type = Forest. Check that UV2 = Monitoring Type
-   Filter for Type = Forest. Check that UV3 = Vegetation Association

#### Combine Comments Errors {.hidden}

```{r}
# Combine
errors_MacroplotReport_Comments <- unique(rbind(errors_MacroplotReport_Quotes))

remove(errors_MacroplotReport_Quotes)

if(nrow(errors_MacroplotReport_Comments) > 1) {
  errors_MacroplotReport_Comments <- errors_MacroplotReport_Comments %>%
    filter(Error != "No Error")
} else {
  errors_MacroplotReport_Comments <- errors_MacroplotReport_Comments}
```
...
```{r}
errors_MacroplotReport <- rbind(errors_MacroplotReport_Installation, errors_MacroplotReport_Location, errors_MacroplotReport_SampleEvent, errors_MacroplotReport_Comments)
```
...
```{r}
errors_ProjectUnitReport <- unique(rbind(errors_ProjectUnitReport_Quotes, errors_ProjectUnitReport_Macroplot, errors_ProjectUnitReport_ProjectUnit, errors_ProjectUnitReport_MultiPU))

remove(errors_ProjectUnitReport_Quotes, errors_ProjectUnitReport_Macroplot, errors_ProjectUnitReport_ProjectUnit, errors_ProjectUnitReport_MultiPU)
...
```{r}
errors_SampleEventReport <- unique(rbind(errors_SampleEventReport_Quotes, errors_SampleEventReport_AdministrationUnit, errors_SampleEventReport_Protocols, errors_SampleEventReport_Visited_1, errors_SampleEventReport_Visited_2, errors_SampleEventReport_MonStatus))

remove(errors_SampleEventReport_Quotes, errors_SampleEventReport_AdministrationUnit, errors_SampleEventReport_Protocols, errors_SampleEventReport_Visited_1, errors_SampleEventReport_Visited_2, errors_SampleEventReport_MonStatus)
...
```{r}
errors_all <- rbind(errors_MacroplotReport, errors_blank, errors_ProjectUnitReport, errors_blank, errors_SampleEventReport)
```
