...
path_main_data <- paste0(path_main, "Exports/FFI/")
path_save <- paste0(path_main, "Exports_Clean/FFI/")
```
...
for(i in parks) {
  ...
  MacroplotReport_MetaData <- MacroplotReport %>%
    ...
    separate(DateIn, sep = " ", 
             into = c("Macroplot_DateIn")) %>%
    separate(FutureVisit, sep = " ", 
             into = c("Macroplot_FutureVisit")) %>%
    separate(DateOut, sep = " ", 
             into = c("Macroplot_DateOut")) %>%
  ...
  MacroplotReport_SampleEvents <- MacroplotReport %>%
    ...
    select(c(Macroplot_Name, SampleEvent_Date, 
             ...
             SampleEvent_LegacyMonitoringStatus))
  ...
  SampleEventReport_clean <- SampleEventReport %>%
    ...
    # remove unnecessary columns
    select(!c(SampleEvent_LegacyMonitoringStatus))
  ...
  MetadataReport <- merge(MacroplotReport_merge, SampleEventReport_merge, by = c("Macroplot_Name", "SampleEvent_Date")) %>%
...
# final column renaming to match protocol exports
  ...
  # save file
write.csv(final_df, paste0(path_save, "all_metadata.csv"), quote = FALSE, row.names = FALSE) 
```
...
for(i in parks) {
  ...
  # save file
  write.csv(final_df, paste0(path_save, "all_species_info.csv"), quote = FALSE, row.names = FALSE)
```
...
```{r cover_line_intercept, warning=FALSE}
...
for(i in parks) {
  ...
  if (all(is.na(HerbsPoints_all[[ncol(HerbsPoints_all)]]))) {
    HerbsPoints_all <- HerbsPoints_all[, -ncol(HerbsPoints_all), drop = FALSE]
  }
  ## replace commas with semicolons
  ...
  HerbsPoints_all <- HerbsPoints_all %>%
    ...
    mutate(across(where(is.character), ~ gsub("[\r\n]+", " ", .))) %>% 
    # separate Date column
    separate(Date, sep = " ", into = c("SampleEvent_Date")) %>%
    # reformate Date column
    mutate(SampleEvent_Date = as.Date(SampleEvent_Date, "%m/%d/%Y"))
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_cover_points.csv"), quote = FALSE, row.names = FALSE)
```
...
# final column renaming to match metadata
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_cover_species_composition.csv"), quote = FALSE, row.names = FALSE) 
```
...
# final column renaming to match metadata
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_density_belts.csv"), quote = FALSE, row.names = FALSE)
```
...
# final column renaming to match metadata
  final_df <- final_df %>%  
    dplyr::rename(
      ...
      MethodData_QuadLen_m = QuadLen,
      MethodData_QuadWid_m = QuadWid,
      MethodData_Area_ha = Area,
      MethodData_FieldTeam = FieldTeam,
      MethodData_EntryTeam = EntryTeam,
      MethodData_UV1Desc = UV1Desc,
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_density_quadrats.csv"), quote = FALSE, row.names = FALSE)
```
...
```{r surface_fuels_alaska_duff_litter, warning=FALSE}
...
# final column renaming to match metadata
  ...
  # save file
  write.csv(final_df, paste0(path_save, "english_surface_fuels_duff_litter.csv"), quote = FALSE, row.names = FALSE) 
```
...
# final column renaming to match metadata
  ...
  # save file
  write.csv(final_df, paste0(path_save, "english_surface_fuels_fine.csv"), quote = FALSE, row.names = FALSE) 
```

### surface_fuels_piles

```{r surface_fuels_piles, warning=FALSE}
...
for(i in parks) {
  ...
  # save file
  write.csv(final_df, paste0(path_save, "english_surface_fuels_thousand_hr.csv"), quote = FALSE, row.names = FALSE)
```

### surface_fuels_vegetation

```{r surface_fuels_vegetation, warning=FALSE}
...
# final column renaming to match metadata
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_trees_individuals.csv"), quote = FALSE, row.names = FALSE)
```

### trees_saplings

```{r trees_saplings, warning=FALSE}

```

### trees_seedlings

```{r trees_seedlings, warning=TRUE}
...
# final column renaming to match metadata
  final_df <- final_df %>% 
    dplyr::rename(
      ...
      MethodData_Transect = Transect,
      MethodData_Point = Point,
      MethodData_TapeDist_m = TapeDist,
      MethodData_Veg = Veg,
      MethodData_Sub = Sub,
      MethodData_Comment = Comment,
      MethodData_UV1 = UV1,
  ...
  # save file
  write.csv(final_df, paste0(path_save, "metric_post_burn_severity.csv"), quote = FALSE, row.names = FALSE) 
```
