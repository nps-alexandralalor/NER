...
calculate_coarse_fuels <- function(fuels_df) {
  ...
  fuels_df <- fuels_df %>% 
    mutate(
      SampleEvent_Date = ymd(SampleEvent_Date),
  ...
  loads_per_transect <- fuels_processed %>% 
   group_by(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Transect) %>% 
    dplyr::reframe(
  ...
  all_transects <- fuels_df %>%
    select(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, NumTran, Transect, TranLen) %>% 
  ...
  fuel_by_transect <- all_transects %>%
    left_join(loads_per_transect,
              by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Transect", "ThousHrTranLen")) %>%
    mutate(
...
calculate_litterduff <- function(fuels_df) {
  ...
  transect_means <- fuels_df %>%
    group_by(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Transect, NumTran) %>% 
    dplyr::reframe(
...
Fuels1000_all <- Fuels1000 %>% 
  select(!c(Macroplot_UV1, Macroplot_UV2, Macroplot_UV3, Macroplot_UV4, Macroplot_UV5, Macroplot_UV6, Macroplot_UV7, Macroplot_UV8, MonitoringStatus_Prefix, MonitoringStatus_Base, MonitoringStatus_Suffix, MonitoringStatus_Order, SampleEvent_Date_Year, FieldTeam, EntryTeam, UV1Desc, UV2Desc, UV2Desc, UV3Desc, SaComment, Visited, Index)) %>% 
  mutate(SampleEvent_Date = ymd(SampleEvent_Date))
...
Fuels_calc <- FuelsFine_sum %>%
  dplyr::full_join(FuelsDuffLitt_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Transect")) %>% 
  dplyr::full_join(Fuels1000_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Transect")) %>% 
...
```{r}
write.csv(Fuels_calc, paste0(path_save, "Data_Fuels.csv"), quote = FALSE, row.names = FALSE)
```
...
calculate_trees_dens <- function(trees_df) {
  ...
  # ---- Step 2: Compute useful values ---- #
  plot_level <- trees_df %>%
    mutate(
      Hectar = (SubFrac * 1000) / 10000,
      Acre = Hectar * 2.47105,
      SizeCl = if_else(DBH < 15.0, "Pole", "Overstory"),
      Ht_m = Ht,
      Ht_ft = round((Ht * 3.28084), 2),
      DBH_cm = DBH,
      DBH_in = round((DBH * 0.39370079), 2),
  ...
  plot_level_pole <- plot_level %>% 
    filter(SizeCl == "Pole") %>% 
    group_by(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Genus, ScientificName, Status) %>% 
    dplyr::reframe(
  ...
  plot_level_merge <- plot_level %>% 
    select(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Genus, ScientificName, Status) %>% 
    unique()
  
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level_pole, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
    full_join(plot_level_overstory, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
    mutate(
...
calculate_seedlings_dens <- function(seedlings_df) {
  ...
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level_seedling, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
    full_join(plot_level_sapling, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
    mutate(
...
```{r}
Trees_meta <- Trees %>% 
  select(1:20, Genus, ScientificName, Status) %>% 
  unique()
...
Trees_calc <- Trees_meta %>% 
  dplyr::full_join(Trees_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
  unique()


Seedlings_calc <- Seedlings_meta %>% 
  dplyr::full_join(Seedlings_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Genus", "ScientificName", "Status")) %>% 
  unique()
```
...
calculate_Points_cover <- function(cover_df) {
  ...
  cover_df <- cover_df %>%
    mutate(
      ...
      Nativity = ifelse(Native.Species == "True", "Native",
                           ifelse(Invasive.Species == "True", "Invasive", 
                                  ifelse(Preferred_Lifeform != "Undefined", 
                                         "Non-Native", "Unknown")))
    )
  ...
  plot_level <- cover_df %>%
    group_by(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name) %>% 
    dplyr::reframe(
      n_species_plot = n_distinct(ScientificName)
      ) %>% 
    unique()
  ## ------------- By Species ------------ ##
  plot_level_species <- cover_df %>% 
    mutate(NumPtsTran = ifelse(NumPtsTran == 0, 100, NumPtsTran)) %>% 
    group_by(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Preferred_Lifeform, Genus, ScientificName, Nativity) %>% 
    dplyr::reframe(
      #Species = paste(na.omit(unique(Species.Symbol)), collapse = "; "),
      Hits = n(),
      PctCover = round(((Hits / (NumPtsTran * NumTran)) * 100), 1),
      Avg_Ht_m = round(mean(Height, na.rm = TRUE), 2),
  ...
  plot_level_merge <- cover_df %>%
    select(AdminUnit_Name, Macroplot_Purpose, Macroplot_Type, Macroplot_Name, MonitoringStatus_Name, Preferred_Lifeform, Genus, ScientificName, Nativity) %>%
    unique()
  
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name")) %>% 
    full_join(plot_level_species, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Preferred_Lifeform", "Genus", "ScientificName", "Nativity"))
  ...
  mutate(
    Nativity = ifelse(Native.Species == "True", "Native",
                      ifelse(Invasive.Species == "True", "Invasive",
                             ifelse(Preferred_Lifeform != "Undefined",
                                    "Non-Native", "Unknown")))) %>%
  select(1:20, Preferred_Lifeform, Genus, ScientificName, Nativity) %>%
  unique()
...
Cover_Points_calc <- Cover_Points_meta %>%
  dplyr::full_join(HerbsPoints_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Preferred_Lifeform", "Genus", "ScientificName", "Nativity"))

Cover_SpComp_calc <- Cover_SpComp_meta %>%
  dplyr::full_join(HerbsSpComp_sum, by = c("AdminUnit_Name", "Macroplot_Purpose", "Macroplot_Type", "Macroplot_Name", "MonitoringStatus_Name", "Preferred_Lifeform", "Genus", "ScientificName", "Nativity"))
```

```{r}
write.csv(Cover_Points_calc, paste0(path_save, "Data_Cover_Points.csv"), quote = FALSE, row.names = FALSE)
write.csv(Cover_SpComp_calc, paste0(path_save, "Data_Cover_SpComp.csv"), quote = FALSE, row.names = FALSE)
```
