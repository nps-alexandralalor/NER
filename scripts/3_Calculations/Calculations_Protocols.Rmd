...
calculate_trees_dens <- function(trees_df) {
  ...
  plot_level_pole <- plot_level %>% 
    filter(SizeCl == "Pole") %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    dplyr::summarise(
    ...
    unique()
  ### ------------- Live -------------- ###
  plot_level_pole_L <- plot_level %>% 
    filter(SizeCl == "Pole", Status == "L") %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    dplyr::summarise(
      L_Pole_Count = n_distinct(TagNo),
      L_Pole_Species = paste(na.omit(unique(Species.Symbol)), collapse = "; "),
      L_Pole_Dens_ha = round(L_Pole_Count / Hectar),
      L_Pole_Dens_ac = round(L_Pole_Count / Acre),
      .groups = "drop"
    ) %>% 
    unique()
  ### ------------- Dead -------------- ###
  plot_level_pole_D <- plot_level %>% 
    filter(SizeCl == "Pole", Status == "D") %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    dplyr::summarise(
      D_Pole_Count = n_distinct(TagNo),
      D_Pole_Species = paste(na.omit(unique(Species.Symbol)), collapse = "; "),
      D_Pole_Dens_ha = round(D_Pole_Count / Hectar),
      D_Pole_Dens_ac = round(D_Pole_Count / Acre),
      .groups = "drop"
    ) %>% 
    unique()
  ### ------------- Merge -------------- ###
  plot_level_pole_merge <- plot_level_pole %>% 
    full_join(plot_level_pole_L, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    full_join(plot_level_pole_D, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus"))
  ...
  plot_level_overstory <- plot_level %>% 
    ...
    unique()
  ### ------------- Live -------------- ###
  plot_level_overstory_L <- plot_level %>% 
    filter(SizeCl == "Overstory", Status == "L") %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    dplyr::summarise(
      L_Overstory_CrwnCl = paste(na.omit(unique(CrwnCl)), collapse = "; "),
      L_Overstory_Count = n_distinct(TagNo),
      L_Overstory_Species = paste(na.omit(unique(Species.Symbol)), collapse = "; "),
      L_Overstory_Dens_ha = round(L_Overstory_Count / Hectar),
      L_Overstory_Dens_ac = round(L_Overstory_Count / Acre),
      .groups = "drop"
    ) %>% 
    unique()
  ### ------------- Dead -------------- ###
  plot_level_overstory_D <- plot_level %>% 
    filter(SizeCl == "Overstory", Status == "D") %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    dplyr::summarise(
      D_Overstory_CrwnCl = paste(na.omit(unique(CrwnCl)), collapse = "; "),
      D_Overstory_Count = n_distinct(TagNo),
      D_Overstory_Species = paste(na.omit(unique(Species.Symbol)), collapse = "; "),
      D_Overstory_Dens_ha = round(D_Overstory_Count / Hectar),
      D_Overstory_Dens_ac = round(D_Overstory_Count / Acre),
      .groups = "drop"
    ) %>% 
    unique()
  ### ------------- Merge -------------- ###
  plot_level_overstory_merge <- plot_level_overstory %>% 
    full_join(plot_level_overstory_L, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    full_join(plot_level_overstory_D, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus"))
  
  
  # ---- Step 4: COMBINE ---- #
  plot_level_merge <- plot_level %>% 
    select(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Genus) %>% 
    unique()
  
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level_pole_merge, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    full_join(plot_level_overstory_merge, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    mutate(
      Pole_Count     = replace_na(Pole_Count, 0),
      Pole_Dens_ha   = replace_na(Pole_Dens_ha, 0),
      Pole_Dens_ac   = replace_na(Pole_Dens_ac, 0),
      L_Pole_Count     = replace_na(L_Pole_Count, 0),
      L_Pole_Dens_ha   = replace_na(L_Pole_Dens_ha, 0),
      L_Pole_Dens_ac   = replace_na(L_Pole_Dens_ac, 0),
      D_Pole_Count     = replace_na(D_Pole_Count, 0),
      D_Pole_Dens_ha   = replace_na(D_Pole_Dens_ha, 0),
      D_Pole_Dens_ac   = replace_na(D_Pole_Dens_ac, 0),
      ...
      L_Overstory_Count      = replace_na(L_Overstory_Count, 0),
      L_Overstory_Dens_ha    = replace_na(L_Overstory_Dens_ha, 0),
      L_Overstory_Dens_ac    = replace_na(L_Overstory_Dens_ac, 0),
      D_Overstory_Count      = replace_na(D_Overstory_Count, 0),
      D_Overstory_Dens_ha    = replace_na(D_Overstory_Dens_ha, 0),
      D_Overstory_Dens_ac    = replace_na(D_Overstory_Dens_ac, 0)
    )
}
```
...
calculate_seedlings_dens <- function(seedlings_df) {
  ...
  plot_level_total <- plot_level %>% 
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus) %>% 
    dplyr::summarise(
      Total_SeedCount = sum(Count),
      Total_SeedDens_ha = round(Total_SeedCount / Hectar),
      Total_SeedDens_ac = round(Total_SeedCount / Acre),
      .groups = "drop") %>% 
    unique()
  ...
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level_total, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus")) %>% 
    full_join(plot_level_seedling, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    full_join(plot_level_sapling, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
    mutate(
...
Trees_Seedlings <- Trees_meta %>% 
  dplyr::full_join(Trees_sum, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
  dplyr::full_join(Seedlings_sum, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Genus")) %>% 
  unique()
```
...
calculate_SpComp_cover <- function(cover_df) {
  ...
  plot_level <- cover_df %>%
    ...
    group_by(AdministrationUnit_Name, Macroplot, MonStatus, SampleEventDate_Year, Nativity) %>%
    dplyr::mutate(
      n_species_ByPlotNativity = n_distinct(Species.Symbol)) %>%
    group_by(AdministrationUnit_Name, Macroplot, MonStatus, SampleEventDate_Year, Preferred_Lifeform) %>%
    dplyr::mutate(
      n_species_ByPlotLifeform = n_distinct(Species.Symbol)) %>%
    # ---- KEEP BURN UNIT-LEVEL DATA (do NOT summarise here) ---- #
    group_by(AdministrationUnit_Name, MonStatus, SampleEventDate_Year, Nativity) %>%
    dplyr::mutate(
