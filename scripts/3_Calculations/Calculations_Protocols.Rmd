...
Fuels1000_all <- Fuels1000 %>% 
  select(!c(Macroplot_UV1, Macroplot_UV2, Macroplot_UV3, Macroplot_UV4, Macroplot_UV5, Macroplot_UV6, Macroplot_UV7, Macroplot_UV8, Status.Prefix, Status.Base, Status.Suffix, MonStatusOrd, SampleEventDate_Year, FieldTeam, EntryTeam, UV1Desc, UV2Desc, UV2Desc, UV3Desc, SaComment, Visited, Index)) %>% 
...
calculate_Points_cover <- function(cover_df) {
  ...
  plot_level <- cover_df %>%
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus) %>% 
    dplyr::summarise(
      n_species_plot = n_distinct(Species.Symbol),
  ...
  plot_level_species <- cover_df %>% 
    ...
    group_by(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Preferred_Lifeform, Genus, Nativity, Species.Symbol) %>% 
    dplyr::summarise(
  ...
  plot_level_merge <- cover_df %>%
    select(AdministrationUnit_Name, Purpose, Type, Macroplot, MonStatus, Preferred_Lifeform, Genus, Nativity, Species.Symbol) %>%
    unique()
  
  plot_level_sum <- plot_level_merge %>% 
    full_join(plot_level, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus")) %>% 
    full_join(plot_level_species, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Preferred_Lifeform", "Genus", "Nativity", "Species.Symbol"))
  ...
  mutate(
    Nativity = ifelse(Native.Species == "True", "Native",
                      ifelse(Invasive.Species == "True", "Invasive",
                             ifelse(Preferred_Lifeform != "Undefined",
                                    "Non-Native", "Unknown")))) %>%
  select(1:20, Preferred_Lifeform, Genus, Nativity, Species.Symbol) %>%
  unique()
...
```{r}
Cover_Points <- Cover_Points_meta %>%
  dplyr::full_join(HerbsPoints_sum, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Preferred_Lifeform", "Genus", "Nativity", "Species.Symbol"))

Cover_SpComp <- Cover_SpComp_meta %>%
  dplyr::full_join(HerbsSpComp_sum, by = c("AdministrationUnit_Name", "Purpose", "Type", "Macroplot", "MonStatus", "Preferred_Lifeform", "Genus", "Nativity", "Species.Symbol"))
```

```{r}
write.csv(Cover_Points, paste0(path_save, "Data_Cover_Points.csv"), quote = FALSE, row.names = FALSE)
write.csv(Cover_SpComp, paste0(path_save, "Data_Cover_SpComp.csv"), quote = FALSE, row.names = FALSE)
```
