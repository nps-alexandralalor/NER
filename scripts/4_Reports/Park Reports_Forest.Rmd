...
date_min <- as.Date(min(MetadataReport_graph$SampleEventDate))
    ...
    MetadataReport_graph %>% 
      ...
      select(Macroplot_UV1) %>% 
      unique()

target_project <- c("Cold Harbor I")
...
p_fuels <- Fuels_graph %>% 
  ...
  geom_vline(xintercept = (MetadataReport_graph$DisturbanceDate_Year) - 0.5, linetype = "dashed", color = "red")
...
p_trees_classic <- Trees_graph %>% 
  ...
  group_by(
    ...
    SampleEvent_TreatmentUnit, MonStatus, SampleEventDate_Year
    ) %>% 
