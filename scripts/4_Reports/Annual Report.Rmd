...
```{r}
## Adjust File Paths
path_data <- "E:/Allie/FireFX_NER/FFI Data Management/R Outputs/Calculations"
#path_data <- "C:/Users/alalor/OneDrive - DOI/NER/FireFX/FFI Data Management/R Outputs/Calculations"
```
...
unique(MetadataReport_all$SampleEventDate_Year)
target_year <- c(2025)
```
...
table_ProjectUnit <- MetadataReport_all %>% 
  group_by(AdminUnit_Name, Plot_Type, MacroPlot.UV1) %>% 
  dplyr::summarise(MacroPlot.UV2 = paste(na.omit(unique(MacroPlot.UV2)), collapse = ", "),
                   ...
                   Total_Plots_ByPark = mean(n_plots_ByPlotType),
                    .groups = "drop")

table_MonitoringType <- MetadataReport_all %>% 
  group_by(AdministrationUnit_Name, Plot_Type, MacroPlot.UV2) %>% 
  dplyr::summarise(MacroPlot.UV1 = paste(na.omit(unique(MacroPlot.UV1)), collapse = ", "),
                   First_Year = min(SampleEventDate_Year),
                   Last_Year = max(SampleEventDate_Year),
                   Annual_Total = n_distinct(Macroplot[SampleEventDate_Year == target_year]),
                   Total_Plots_ByProject = n_distinct(Macroplot),
                   Total_Plots_ByPark = mean(n_plots_ByPlotType),
                    .groups = "drop") %>% 
  dplyr::rename(MacroPlot.UV1 = MacroPlot.UV2, MacroPlot.UV2 = MacroPlot.UV1)
```
...
table1_w_totals_final <- table1_w_totals %>% 
  select(AdministrationUnit_Name, MacroPlot.UV1, Plot_Type, Years, Annual_Total, Total_Plots) %>%
  dplyr::rename("Park" = AdministrationUnit_Name,
                "Plot Type" = Plot_Type,
                "Monitoring Unit" = MacroPlot.UV1,
...
tableB1 <- table_ProjectUnit %>% 
  ...
  mutate(MacroPlot.UV1 = ifelse(MacroPlot.UV2 != "", 
                                paste0(MacroPlot.UV1, "<br>(", MacroPlot.UV2, ")"), 
                                MacroPlot.UV1)) %>% 
...
tableB1_w_totals_final <- tableB1_w_totals %>% 
  select(AdministrationUnit_Name, MacroPlot.UV1, Plot_Type, Years, Total_Plots) %>%
  dplyr::rename("Park" = AdministrationUnit_Name,
                "Plot Type" = Plot_Type,
                "Monitoring Unit" = MacroPlot.UV1,
                "Years Data Collected<br>(start-end)" = Years,
