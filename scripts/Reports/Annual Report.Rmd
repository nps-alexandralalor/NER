...
table1 <- MetadataReport %>% 
  ...
  group_by(AdministrationUnit_Name, Purpose, Type, n_total_plots) %>% 
  
  dplyr::summarise(first_year = min(SampleEventDate_Year),
            last_year = max(SampleEventDate_Year),
            n_current_yr_plots = n_distinct(Macroplot[SampleEventDate_Year == format(Sys.Date(), "%Y")])) %>%  # designate current year
  mutate("Plot Type" = ifelse(Purpose == "Fire Monitoring Handbook", 
                              ...
                              ifelse(Purpose == "Rapid Assessment", 
                                     ...
                                     ifelse(Purpose == "Inventory & Monitoring", 
                                            "I&M - Oak Regen Research Plots", Type)))) %>% 
