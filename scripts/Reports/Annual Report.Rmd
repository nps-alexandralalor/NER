...
table1 <- MetadataReport %>% 
  ...
  group_by(AdministrationUnit_Name, Purpose, Type, UV1) %>% 
  dplyr::summarise(first_year = min(SampleEventDate_Year),
            last_year = max(SampleEventDate_Year),
            n_total_plots = n_distinct(Macroplot),
            n_current_yr_plots = n_distinct(Macroplot[SampleEventDate_Year == format(Sys.Date(), "%Y")])) %>%  # designate current year
  mutate("Plot Type" = ifelse(Purpose == "Fire Monitoring Handbook", paste0("FMH - ", Type), 
                              ifelse(Purpose == "Rapid Assessment", paste0("RA - ", Type), 
                                     ifelse(Purpose == "Inventory & Monitoring", "I&M - Oak Regeneration Research Plots", Type)))) %>% 
