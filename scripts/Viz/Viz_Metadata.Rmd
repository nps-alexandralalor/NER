...
graph_sum_Disturbances %>% 
  ...
  ggplot(aes(x = DisturbanceDate, y = Type)) +
  geom_line(aes(group = Type)) +
  geom_point(aes(fill = Disturbance_Type, size  = n_Macroplot), shape = 21) +
  #geom_text(size = 4) +
...
graph_MetadataReport %>% 
  ...
  geom_point(aes(group = graph_Disturbances$Disturbance_Type, fill = graph_Disturbances$Disturbance_Type), data = graph_Disturbances, shape = 1, size = 2) +
  facet_grid(Type ~ AdministrationUnit_Name) +
  labs(title = paste0("Sample Event History"),
       subtitle = "Fire Monitoring Handbook (FMH) Plots",
       y = "Plot Number",
