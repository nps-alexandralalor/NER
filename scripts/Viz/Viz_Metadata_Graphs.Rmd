...
graph_sum_Disturbances %>% 
  filter(AdministrationUnit_Name == "RICH",
         Type == "Grassland") %>% 
  ggplot(aes(x = DisturbanceDate, y = MonStatus)) +
  geom_line(aes(group = MonStatus)) +
  geom_point(aes(fill = MonStatus), shape = 21, size = 2) +
...
#   geom_line(aes(group = Macroplot_Num), color = "grey") +
#   geom_point(aes(color = MonStatus)) +
#   facet_grid(Type ~ AdministrationUnit_Name) +
